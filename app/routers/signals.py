...
from app.models.schemas.signal import (
    BinanceSignalDetailResponse,
    BinanceSignalResponse,
...
# ── Binance Signals (/binance-signals) ─────────────────────────────


@router.get("/binance-signals", response_model=PaginatedResponse)
async def list_binance_signals(
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
    symbol: Optional[str] = None,
    direction: Optional[SignalDirection] = None,
    ...
    query = select(BinanceSignal).order_by(BinanceSignal.created_at.desc())
    count_query = select(func.count(BinanceSignal.id))

    if symbol:
        query = query.where(BinanceSignal.symbol == symbol)
        count_query = count_query.where(BinanceSignal.symbol == symbol)
    if direction:
        query = query.where(BinanceSignal.direction == direction)
        count_query = count_query.where(BinanceSignal.direction == direction)
    if status:
        query = query.where(BinanceSignal.status == status)
        count_query = count_query.where(BinanceSignal.status == status)
    ...
    return PaginatedResponse(
        items=[BinanceSignalResponse.model_validate(s) for s in signals],
        total=total,
        page=page,
        page_size=page_size,
        total_pages=(total + page_size - 1) // page_size if page_size > 0 else 0,
...
@router.get("/binance-signals/live", response_model=list[BinanceSignalResponse])
async def get_live_binance_signals(
    perms: UserPermissions = RequireSubscribed,
    db: AsyncSession = Depends(get_db),
):
    result = await db.execute(
        select(BinanceSignal)
        .where(BinanceSignal.status.in_([SignalStatus.NEW, SignalStatus.EXECUTING]))
        .order_by(BinanceSignal.confidence.desc())
    )
    signals = result.scalars().all()
    return [BinanceSignalResponse.model_validate(s) for s in signals]


@router.get("/binance-signals/history", response_model=PaginatedResponse)
async def get_binance_signal_history(
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
    perms: UserPermissions = RequireSubscribed,
    db: AsyncSession = Depends(get_db),
    ...
    query = (
        select(BinanceSignal)
        .where(BinanceSignal.status.in_(status_filter))
        .order_by(BinanceSignal.created_at.desc())
    )
    count_query = select(func.count(BinanceSignal.id)).where(
        BinanceSignal.status.in_(status_filter)
    )
...
@router.get("/binance-signals/{signal_id}", response_model=BinanceSignalDetailResponse)
async def get_binance_signal_detail(
    signal_id: uuid.UUID,
    perms: UserPermissions = RequireSubscribed,
    db: AsyncSession = Depends(get_db),
):
    result = await db.execute(
        select(BinanceSignal)
        .options(selectinload(BinanceSignal.executions))
        .where(BinanceSignal.id == signal_id)
    )
    signal = result.scalar_one_or_none()
    if signal is None:
        raise HTTPException(status_code=404, detail="Binance signal not found")
    return BinanceSignalDetailResponse.model_validate(signal)
