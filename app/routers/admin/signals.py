"""Admin signals router — cross-user signal listing and cancellation for both exchanges."""
...
async def admin_list_bybit_signals(
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
    symbol: Optional[str] = None,
    direction: Optional[SignalDirection] = None,
    ...
    if signal_status:
        query = query.where(BybitSignal.status == signal_status)
        count_query = count_query.where(BybitSignal.status == signal_status)
...
async def admin_get_bybit_signal(
    signal_id: uuid.UUID,
    db: AsyncSession = Depends(get_db),
):
    result = await db.execute(
...
@router.put("/bybit-signals/{signal_id}/cancel", response_model=BybitSignalResponse)
async def admin_cancel_bybit_signal(
    signal_id: uuid.UUID,
    request: Request,
    db: AsyncSession = Depends(get_db),
    admin_perms: UserPermissions = RequireAdmin,
):
    result = await db.execute(select(BybitSignal).where(BybitSignal.id == signal_id))
    signal = result.scalar_one_or_none()
    if signal is None:
        raise HTTPException(status_code=404, detail="Bybit signal not found")
    ...
    await log_audit(
        db=db,
        user_id=admin_perms.user.id,
        action="admin_cancel_bybit_signal",
        resource_type="bybit_signal",
        resource_id=str(signal_id),
        ip_address=request.client.host if request.client else None,
    )

    return BybitSignalResponse.model_validate(signal)
